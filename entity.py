...
class DSTForensics:
    ...
    def get_dst_info(self, base_dt: Optional[datetime] = None) -> Dict[str, Any]:
        ...
        return {
            ...
            "direction": direction,
            "message": message,
            "days_to_event": (transition_moment.date() - datetime.now(self.tz).date()).days,
    ...
    def _find_exact_moment(self, base_dt: Optional[datetime] = None) -> datetime:
        ...
        day_start = (ref_dt + timedelta(days=low_day-1)).replace(hour=0, minute=0, second=0, microsecond=0)
